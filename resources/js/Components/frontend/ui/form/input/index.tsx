...
import { checkValidity, classNames } from '@/utils'
...
export default function Input({ icon: Icon, label, addon, append, inputSize = 'lg', className, validation, ...props }: InputProps) {
    ...
    return <div className={className}>
        ...
        <div className={classNames(inputSize === 'sm' ? "h-12" : "h-[60px]", "rounded-[300px] relative bg-neutral-100 flex items-center")}>
            <div>
                {Icon && <div className={classNames(inputSize === 'sm' ? "w-12" : "w-12 md:w-16", "flex relative z-10 justify-center")}>
                    ...
                    {inputSize === 'sm' && <div className='rounded-full w-1 h-1 bg-neutral-700/20 absolute right-1.5 top-1/2 -translate-y-1/2' />}
                </div>}
